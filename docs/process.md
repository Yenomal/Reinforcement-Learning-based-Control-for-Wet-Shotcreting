...
- 进行对比实验后发现：1）MLP输出不了小步伐、2）最后成功率可能是随机噪声带入的（调小ppo最终的policy输出方差），两个问题其实是比较耦合的

PS. 现在看来PPO这种方法最大的特色就是他稳定，稳定就可以上大一点的模型，而且比较适合并行，所以提高模型的收益要远大于使用SAC这种针对性算法的收益

## Version 4

- 在大规模对比实验下我们已经完成了0.003 95%以上的收敛，现在摆在我们面前的是最后一公里问题，如何进一步提高精度并最终达到 100% 精度，以及如何提高模型鲁棒性在多种工况下进行

- 精度进一步提高问题 —— 利用成功率引导的探索度下降，以完成最后的policy收敛，但是发现onpolicy RL的崩溃问题，在低探索度下policy可能在某一个episodes探索到较少的成功样本，用于训练后带偏critic，最终policy彻底崩溃，我们需要一个强有力的**正则化**避免policy崩溃

关于policy在-4.1的崩溃现象：

1. update_epochs 不是当前崩溃的主要控制因素

2. clip_ratio、value_coef 不是当前崩溃主要因素

3. 

- 在仿真环境下直接施加噪声，包括传感器上的高斯噪声+随机mask，应对inference的时候的传感器噪声