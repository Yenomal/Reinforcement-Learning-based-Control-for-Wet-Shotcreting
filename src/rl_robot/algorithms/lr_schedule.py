...
class SuccessRateTriggeredScheduler:
    ...
    def cooldown_total(self) -> int:
        """Fixed cooldown length after each value decay."""
    ...
    def observe(
        ...
        *,
        ...
        if (
            episodes_in_window is not None
            and int(episodes_in_window) < self.min_episodes_in_window
        ):
            return self.state()
...
class CosineThenSuccessRateTriggeredScheduler:
    ...
    def __init__(
        ...
        *,
        ...
        if self.switch_update > 0:
            ...
            self.trigger_scheduler.current_value = max(
                handoff_value,
                self.trigger_scheduler.min_value,
            )
    ...
    def observe(
        ...
        *,
        ...
        if progress is None:
            raise ValueError(
                "CosineThenSuccessRateTriggeredScheduler.observe requires progress."
            )

        if not self.in_trigger_phase(progress):
