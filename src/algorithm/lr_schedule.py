#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Shared learning-rate schedules for PPO and SAC."""
...
class OptimizerLRScheduler:
    """Simple optimizer-level scheduler shared by PPO and SAC."""
    ...
    def step(self, progress: int) -> Dict[str, float]:
        ...
        for name, optimizer_list in self.optimizers.items():
            for optimizer, base_lr_group in zip(optimizer_list, self.base_lrs[name]):
                for param_group, base_lr in zip(optimizer.param_groups, base_lr_group):
