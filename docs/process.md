...
env：pybullet+机械臂assets（参数需要设计一下，尺度*10）四自由度+岩壁环境设置

算法：PPO（后续增加SAC）+课程学习+迭代训练+碰撞检测+归一化稳定性处理，观测空间，动作空间，planner可达性检查湿喷点规划

buffer设计，offline2Online

工况：传感器高斯噪声，末端突然受力（直接采样3个随机数）
